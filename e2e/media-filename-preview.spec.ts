...
// Local-only command: E2E_BASE_URL=http://localhost:3212 E2E_MUTATION_ALLOWED=1 npx playwright test e2e/media-filename-preview.spec.ts --project=chromium
...
function assertLocalMutationTarget() {
  ...
  try {
    ...
    throw new Error(
      `Refusing mutating Home media preview E2E with invalid E2E_BASE_URL: ${rawBaseUrl}`,
    );
  }
  ...
  if (
    ...
    throw new Error(
      `Refusing mutating Home media preview E2E outside localhost/127.0.0.1:3212: ${rawBaseUrl}`,
    );
  }

  if (process.env.E2E_MUTATION_ALLOWED !== "1") {
    throw new Error(
      "Set E2E_MUTATION_ALLOWED=1 to authorize the local mutating Home media preview E2E.",
    );
  }
}

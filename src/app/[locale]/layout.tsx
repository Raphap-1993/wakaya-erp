...
function buildNav(locale: ReturnType<typeof listPublicSiteLocales>[number], keys?: readonly PublicRouteKey[]) {
  const content = getPublicSiteContent(locale);
  const allowedKeys = keys ? new Set(keys) : null;

  return content.labels.nav
    .filter((item) => !allowedKeys || allowedKeys.has(item.key as PublicRouteKey))
    .map((item) => ({
      key: item.key,
      label: item.label,
  ...
  return (
    <main className={styles.page}>
      ...
      <PlayFooter
        ...
        footerNav={buildNav(locale, FOOTER_NAV_KEYS)}
      />
    </main>
  );
}
